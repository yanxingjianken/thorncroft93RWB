...
Columns: lc, track_id, time_iso, day_hour, lon, lat, pv_anom_330,
         area_deg2
    area_deg2 = local-radius (10°) area where the signed anomaly
    crosses the per-polarity threshold, diagnostic only.

Output: outputs/<lc>/tracks/track_centers_{C,AC}.csv
"""
...
def local_area_deg2(vor_frame, thr, polarity, lat0, lon0, lat1d, lon1d,
                     radius_deg=10.0):
...
def run(lc, polarity="C"):
    ...
    src = out_dir / "tracks" / f"tracks_{tag}_top6_pv330.txt"
    tracks = parse_stitchnodes(src)

    THR = CFG.POS_THRESH if polarity == "C" else -CFG.NEG_THRESH
